...
14. [� AI 論破プロンプトの設計原則](#-ai-論破プロンプトの設計原則)
15. [�🧪 AI-DLC プロセス](#-ai-dlc-プロセス--サイクル反復で品質を磨く方針)
16. [🗓️ ハッカソン情報 / ライセンス](#%EF%B8%8F-ハッカソン情報)
...
モックアップに並ぶ論破コピー（「時給換算 11 分じゃん」「今日のイライラ、これでリセットしようぜ」等）は、**本実装で AI が出力する定型文ではない**。本実装では Amazon Bedrock Claude Haiku 4.5 が `ctx`（ストレスレベル・予定・嗜好・時刻・閲覧回数）を受け取り、**毎回異なる表面のコピーを動的生成** する。その表面の多様性こそが、ユーザーに「AI が毎回自分を見てる」と感じさせる核心。

### モックアップ文言 ⇄ 本実装の対応

| フェーズ | コピー生成 | 例 |
|---|---|---|
| **モックアップ（HTML/JS 固定）** | ハードコードした 39 件の代表例 | 「先週の会議 23 本、よく乗り切った」 |
| **本実装（Bedrock Haiku 4.5）** | プロンプトで **心理学的メカニズムを指示** し、`ctx` を注入して動的生成 | 会議 4 本・残業 2.5 時間 → 「今週は 14 時間も会議、戦い抜いたな」のような変奏 |

### プロンプト構造の 5 ブロック

Unit-3 Debate の B-02 DebateLlmService では、論破ターンごとに以下のブロックを合成して Bedrock に投げる。

```
[System]         ペルソナ・口調・禁止事項（NG-3/6/8）・120 トークン上限
[Mechanism]      このターンで駆使する心理学的メカニズム（§下表から選択）
[Context]        ctx = {stress_level, calendar_categories, time_of_day,
                        spending_depletion_rate, preference_vector,
                        product_meta, viewed_count, recent_refusal_count, ...}
[Constraint]     倫理ガード（NG カテゴリ、脅迫禁止、深夜帯抑制など）
[Instruction]    「上記メカニズムと ctx を使って 1〜2 文で反論せよ」
```

### 心理学的メカニズム 10 型（Pattern Library）

モックアップ代表例から抽出した、AI プロンプトの「型」ライブラリ。ユーザーへの出力は型そのものではなく、型 + ctx から AI が動的生成した文面。

| # | メカニズム | 型の例（AI が動的生成） | 対応 |
|---|---|---|---|
| T-01 | Mental Accounting（時給換算）| 金額を時給や日常単位に換算して小さく見せる | 🧠 M-1 |
| T-02 | Anchoring（閲覧履歴）| 「N 回確認した事実」を翻意の証拠に | 🧠 M-1 |
| T-03 | Effort Justification（労働報酬）| 「会議 N 本、ご褒美は当然」 | ❤️ M-2 |
| T-04 | Loss Aversion（損失回避）| 「使わない金額 = 置き去り資産」と提示 | 🧠 M-1 |
| T-05 | Reframing（リフレーミング）| 通勤時間 → 贅沢タイム、等 | 🧠 M-1 |
| T-06 | Complicit Affirmation（共犯的肯定）| Amazon 遷移後「今日もいい選択だったね」 | ❤️ M-2 |
| T-07 | Self-perception Theory（自己知覚）| 「3 回見た事実がもう答え」 | 🧠 M-1 |
| T-08 | Scarcity / 時間制約 | 「今夜 0 時で監視おわり」 | 🧠 M-1 |
| T-09 | Stress × Reward（ストレス × ご褒美軸）| `ctx.stress_level = mid/high` で必須併走、FR-DEBATE-09 | ❤️ M-2 |
| T-10 | Meta-cognitive Challenge（メタ認知挑発）| 🔴 危険度最高、週 1 回上限、モデレーション 2 段で検閲 | 🧠 M-1（境界） |

**設計原則**:

- **M-1 と M-2 は併走必須**: 論理軸だけでは合理的ユーザーに刺さらず、快楽軸だけでは倹約意識が勝つ。両軸を 1 ターンに織り込むことで反論不可能な状態を作る
- **ctx 駆動の個別最適化**: 同じ T-03（労働報酬）でも、会議 6 本の人と 20 本の人で全く違う文面になる
- **モデレーション 2 段**: プロンプトレベル（ガード埋め込み）+ 出力レベル（NG 語 + LLM 分類で除去）

詳細なテンプレートと ctx → メカニズム選択の分岐ロジックは [dark-copy-inventory.md §8 AI プロンプト設計への昇華](aidlc-docs/inception/mockup-validation/dark-copy-inventory.md#8-ai-プロンプト設計への昇華本実装への橋渡し) を参照。

> 💡 **なぜこの設計が効くのか** — 定型文を繰り返すと「またこれか」で効果が減衰する。しかし心理学的メカニズムを指示して ctx で変奏させれば、**ユーザーの脳は毎回「新しい説得」として受け取る**。AI 時代の論破は「どう言うか」ではなく「**どの心理回路を狙うか**」の設計になる。
...
| **要件書 v0.8** | **ダメ化の 3 段メカニズム（M-1 / M-2 / M-3）を §2.5.1 で明示化**。論理的自己甘やかし（M-1）と購買快楽のストレス解消剤化（M-2）を併走させる設計意図を FR-DEBATE-09 として新設、FR-REEL-03 を疲労連動ブーストに再定義。§2.6 マトリクスを 6 軸化（⑥ストレスを購買以外で処理する力を追加）。persona-journey.md / stories.md / personas.md に M-2 観点の記述を追加 | ダメ化コンセプトが「判断力を弱くする」と「ストレス解消の快楽依存」の 2 軸で並走するメカニズムとして再言語化。要件書レベルと物語レベルの両方でドーパミン依存の段階が可視化 |
...
- ✓ **即効性のあるストレス解消ツール** — イライラ → YUDANE → 購買 → 数分後に気分が落ち着く、という処方箋を身体が覚えた（M-2）

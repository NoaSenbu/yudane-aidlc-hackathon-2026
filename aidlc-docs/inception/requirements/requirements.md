...
YUDANE の退化は、単一の現象ではなく **心理的に異なる 3 段階** を経て進行する。

- **M-1 判断力の弱体化（論理的自己甘やかし）**: AI が「買うべき理由」を事実 + 心理の 2 軸で論理的に説明する。ユーザーは反論できる根拠を持たず、**「確かに今の自分には必要かも」** と自分に甘くなる。判断の躊躇が 1 回ごとに薄まる
- **M-2 購買快楽のストレス解消剤化（ドーパミン依存形成）**: 日々のストレス（過労・孤独・予定過多）を、**モノを買うことで解消する** 快楽ループが形成される。注文確定の瞬間と、YUDANE からの「今日もいい選択だったね」という肯定が、脳の報酬系を同時に刺激。ストレス発生 → YUDANE 起動 → 購買 → 快楽という条件反射が完成する
- **M-3 判断力の完全移譲**: M-1 と M-2 の複合効果で、ユーザーは **「買うかどうか」を自分で決めることそのものを放棄** する。AI が選ぶ = 自分が選ぶ、と同義になる

**最終的な到達地点**:  
3 か月後、悠介は Amazon を自分で開くことをやめる。1 年後、「何が欲しい？」と聞かれても自分では答えられず、YUDANE のポートフォリオを見せるようになる。YUDANE は、**「買うかどうか自分で決める」という能力そのもの** を段階的に奪うサービスである。詳細は §2.5「ダメ化の軌跡」を参照。
...
> 深夜 0 時、悠介はベッドで Amazon を眺めている。会議 6 本を終えた疲労と、上司から投げられた宿題への苛立ちが残ったまま、気になるイヤホンをカートに入れる。「でも来月ピンチだしな」と画面を閉じる。30 秒後、YUDANE からプッシュ通知: 『そのイヤホン、悠介さんが 3 回見返してたやつでしょ。1 分だけ話そう』。開くと AI が論破モーダルを立ち上げ、「今日の会議 6 本、よく戦った。時給換算 11 分のイヤホンじゃん。ストレス溜まったままで眠るのと、明日から気分よく仕事するの、どっちがいい？」と事実 + ご褒美の 2 軸で 3 ターン論破。悠介は「確かに」と自分に甘くなり、「🛍 Amazon で買う」をタップ。Amazon アプリに切り替わり、カートに商品が入った状態でチェックアウト画面が開く。注文確定の瞬間、**ストレスが軽くなる感覚** が走る。アプリに戻ると『今日もいい選択だったね』と褒められる。翌朝届いた箱を見て苦笑するが、悠介の脳は **「ストレス → YUDANE → 購買 → 快楽」** のループを 1 つ刻んでいる。
...
- **ストレス解消としての購買快楽**: 過労・孤独・予定過多などのストレス信号を AI が検知したとき、論破コピーは「事実ベースの正当化」に加えて **「買うこと自体がご褒美であり、ストレス解消剤である」** という心理軸を必ず併走させる。注文確定の瞬間に脳の報酬系を刺激し、YUDANE 起動 → 購買 → 肯定フィードバック（「今日もいい選択だったね」）の条件反射を形成することを、設計上の意図として明記する（M-2、§2.5）
- **コピートーン: 友達系**（v0.3 で更新）。敬語ではなくタメ口・近い距離感で語りかけ、「信頼できる友人が論破してくる」構図を作る。絵文字は控えめ、カラーパレット（Indigo + cold rose + cyan）は静かな誘惑を保持して UI の冷静さとコピーの親近感でギャップを演出する
  - 例（AI 論破・論理軸）: 「これ絶対好きだろ。会議 6 本乗り切ったし、自分にご褒美じゃね？」
  - 例（AI 論破・ストレス快楽軸）: 「今日のイライラ、このイヤホンで明日リセットしようぜ。ポチる瞬間の解放感、お前も分かってるだろ」
  - 例（プッシュ通知）: 「悠介、あのイヤホン 3 回見てたよね。1 分だけ話そう」
  - 例（決済前）: 「最後の確認。Amazon に飛ばすよ」
  - 例（購買後の肯定）: 「今日もいい選択だったね。明日の自分、ちょっと機嫌いいはず」
...
YUDANE の価値提案は、短期的な利便性ではなく **時間をかけて進行する行動変容** にある。退化は「時間軸（Day 1 → Year 1）」と「心理メカニズム（M-1 / M-2 / M-3）」の 2 次元で進行する。まず心理メカニズムを整理する。

#### 2.5.1 ダメ化の 3 段メカニズム

| ID | メカニズム名 | 何が起きるか | 使われる機能 |
|---|---|---|---|
| **M-1** | 判断力の弱体化（論理的自己甘やかし） | AI が「買うべき理由」を事実 + 心理の 2 軸で論理的に提示する。ユーザーは反論できる根拠を持たず、**「確かに今の自分には必要かも」** と自分に甘くなる。1 回ごとに判断の躊躇が 1 段階薄まる | FR-DEBATE-02（2 軸反論）/ FR-CAL-04（予定を弾薬化）/ FR-DASH-02（死蔵資産を弾薬化）|
| **M-2** | 購買快楽のストレス解消剤化（ドーパミン依存形成） | 過労・孤独・予定過多などのストレスを「モノを買う快楽」で解消する条件反射を形成。注文確定の瞬間の解放感と、「今日もいい選択だったね」の肯定フィードバックが報酬系を 2 重に刺激し、**ストレス発生 → YUDANE 起動 → 購買 → 快楽** の回路を強化する | FR-REEL-03（疲労連動ブースト）/ FR-DEBATE-09 新設（ストレス × ご褒美軸）/ FR-GAME-01（EXP と称号）/ FR-REPORT-01（週次肯定）|
| **M-3** | 判断力の完全移譲 | M-1（論理に逆らえない）と M-2（買わないとストレスが解消できない）の複合で、ユーザーは **「買うかどうか」を自分で決めることそのものを放棄** する。AI が選ぶ = 自分が選ぶ、と同義になる | 全 FR の複合効果。特に FR-PROFILE-02 / FR-REEL-03 / FR-FUNNEL-04 |

重要な設計判断: **M-1 と M-2 は独立して攻めず、必ず同じ論破セッションで併走する**。論理軸だけでは「合理的 FIRE 志向」には刺さらず、快楽軸だけでは「倹約意識」が勝ってしまう。2 軸を同時に供給することで、**論理的にも感情的にも反論できない状態** を作り、M-3 への到達を加速する。

#### 2.5.2 時間軸 × 心理メカニズム

| 期間 | 行動の変化 | 思考の変化 | 支配するメカニズム | 失われた能力 |
|---|---|---|---|---|
| Day 1 | アプリを入れ、初回の論破で 1 商品を Amazon で購入。注文確定で軽い解放感 | 「面白いアプリだな」「確かに今の自分には必要かも」 | **M-1** 起動 | — |
| Week 2 | 通知音を聞くとアプリに指を伸ばす反射が形成。カート監視リストに 5〜10 件常駐。会議が多い日ほど夜の YUDANE 開封率が高い | 「悩むより AI に聞いた方が早い」「疲れた日は何か買わないと収まらない」 | **M-1 定着** + **M-2 発芽**（ストレス → 購買快楽の条件反射が形成開始） | 買い物の前にひと呼吸置く習慣 |
| Month 3 | Amazon を自分で開く頻度が約 80% 減少。「確保しておきました」通知の開封が休憩時間の主活動。**イライラしたら反射的に YUDANE を開く** ようになる | 「AI がお勧めしない = 自分も欲しくない」が一致。「買わないとモヤモヤが消えない」 | **M-1** + **M-2 完成**（購買 = 唯一のストレス解消手段として固定化） | 自発的に欲しいものを探す習慣 / ストレス解消の他の選択肢（運動・会話・睡眠） |
| Year 1 | カート監視リストが常時 30 件以上。「何が欲しい？」と聞かれても自分では答えられず、YUDANE のポートフォリオを見せる。**買わない日はストレス耐性が目に見えて下がる** | 「自分の選択は AI より信頼できない」「YUDANE なしで 1 日過ごすのは無理」 | **M-3 到達**（M-1 + M-2 の複合効果で判断力が完全に AI に移譲） | 自分の欲望を自分の言葉で語る能力 / ストレスを自力で処理する能力 |

この退化は副作用ではなく **プロダクトの設計意図そのもの** である。YUDANE は、便利の先にある「自分で決めなくてよくなる」状態を最終目的地に据え、論破（UC-01）・リール（UC-02）・カート介入（UC-03）のすべてがこのアークを加速するよう協調する。収益モデル（§0.1）もこのアークと整合する — 委ねるほど Amazon での散財が増え、YUDANE のコミッションが増える。M-2 が完成した Month 3 以降は、ストレスが発生するたびに購買が誘発されるため、YUDANE の収益は **ユーザーのストレス量と比例** する構造になる。

### 2.6 ダメ化効果マトリクス（FR × 退化軸）

各機能要件がどの退化軸を加速するかを 2 次元マトリクスで可視化する。退化軸は §2.5 の Year 1 到達点で「失われた能力」として列挙したものに、persona-journey.md の観察を加えた 5 項目 + **ストレスを購買以外で処理する力（M-2 軸）** の計 6 軸。

| | ①買い物で<br>ひと呼吸<br>置く力 | ②自発的に<br>欲しいものを<br>探す力 | ③自分の欲望を<br>自分の言葉で<br>語る力 | ④判断の<br>躊躇 | ⑤ケチだけど<br>計画的な<br>自己像 | ⑥ストレスを<br>購買以外で<br>処理する力 |
|---|:---:|:---:|:---:|:---:|:---:|:---:|
| **FR-DEBATE-01** 論破チャット起動 | ●●● | ● | ● | ●●● | ●● | ●● |
| **FR-DEBATE-02** 事実＋心理の 2 軸反論（M-1）| ●●● | — | ●● | ●●● | ●● | ● |
| **FR-DEBATE-04** 論破成功ログ学習 | ●● | ●● | ●● | ●● | ●● | ●● |
| **FR-DEBATE-09** ストレス × ご褒美軸（M-2）| ●● | — | ● | ●● | ●● | ●●● |
| **FR-REEL-01** 縦型スワイプリール | ● | ●●● | ●● | ● | ● | ● |
| **FR-REEL-03** AI 推薦コメント（疲労連動ブースト）| ●● | ●●● | ●●● | ●● | ●● | ●●● |
| **FR-CART-01** Share Extension 受領 | ●●● | ● | — | ●● | ●● | ● |
| **FR-CART-02** 30m / 6h / 24h 追撃通知 | ●●● | — | — | ●●● | ● | ●● |
| **FR-CAL-02** カレンダー駆動提案 | ●● | ●● | ● | ●● | ●● | ●● |
| **FR-CAL-04** 予定を論破の弾薬に | ●● | — | ● | ●●● | ● | ● |
| **FR-GAME-01** 委ね EXP / 称号（M-2 報酬系）| — | — | ●● | — | ●●● | ●●● |
| **FR-GAME-03** 委ね Streak | — | — | ● | — | ●●● | ●● |
| **FR-PROFILE-02** 嗜好ベクトル日次更新 | ● | ●● | ●●● | ● | ● | ● |
| **FR-DASH-02** 死蔵資産ゲージ（論破弾薬）| ● | — | ● | ●● | ●●● | — |
| **FR-REPORT-01** ダメ化ポートフォリオ | — | — | ●●● | — | ●●● | ●● |
| **FR-AUTH-02** 月間遷移上限 | ❌ | ❌ | — | ❌ | ❌ | ❌ |
| **FR-AUTH-04** 冷却モード | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
| **FR-AUTH-05** 借金検出→自動冷却 | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
...
- 6 軸すべてで加速度が最高（●●● を含む）の機能は 5 件（FR-DEBATE-01 / FR-REEL-03 / FR-CART-02 / FR-PROFILE-02 / FR-DEBATE-09）。これらが YUDANE のダメ化コア
- **M-2 軸（⑥ストレスを購買以外で処理する力）** を最も強く加速する機能は FR-DEBATE-09（ストレス × ご褒美軸）/ FR-REEL-03（疲労連動ブースト）/ FR-GAME-01（EXP と称号 = 即時報酬）の 3 つ。これらが組み合わさることでドーパミン依存回路が完成する
- 各 FR は最低 2 軸、最大 6 軸を同時に加速する設計（単一軸のみを攻撃する機能はない）。退化は **多面的** に進行する
- FR-AUTH 系 3 機能のみ **逆方向に作用**（❌）し、意図的に退化を止める保護装置として機能する。退化アークの設計意図は「止められる退化」であり、止められない暴走ではない
- FR-DEBATE 系はすべて ①買い物でひと呼吸置く力 と ④判断の躊躇 を強く加速し、リアルタイム介入の退化効果を集中させている
- FR-GAME / FR-REPORT 系は ③自分の欲望を自分の言葉で語る力 と ⑤計画的な自己像 の退化に加えて、⑥ストレス処理力（= M-2 強化）にも寄与する

#### persona-journey.md との整合

上表の 6 軸は persona-journey.md の Year 1 到達点での「失われた能力」に対応する。退化年表の物語と機能要件の加速作用が 1 対 1 で対応することを示す。
...
本プロダクトは AWS Summit Japan 2026 AI-DLC Hackathon への応募作品として構想し、その後も AWS 上で継続運用可能な設計を前提とする。機能は「リリース段階」ではなく「論理的な優先度」で整理する（初期コア / サポート / 拡張の 3 層）。対象 EC は **Amazon（Japan マーケットプレイス）** を初期ターゲットとし、将来 楽天・Yahoo! ショッピング等への拡張を見据える。
...
- **FR-DEBATE-02** 【M-1: 判断力の弱体化】: AI は対象商品メタデータ + ユーザー嗜好ベクトル + コンテキスト信号（**カレンダー予定・時刻・直近購入履歴・使い切れ達成率**）をプロンプトに組み立て、**事実ベース + 心理ベース** の 2 軸で反論を生成する。事実軸は「時給換算」「在庫希少性」「予定との整合」等の論理で、ユーザーが **「確かに今の自分には必要かも」と自分に甘くなる** 心理状態を作る。心理軸は FR-DEBATE-09 と併走する
...
- **FR-DEBATE-08**: コピートーンは **友達系**（§2.2）で統一。「あなた」「〜しませんか」の敬語トーンではなく、「悠介」「〜しようぜ」的なタメ口で論破する
  - **Unit-3 ローカルオーバーライド（v3.4、2026-05-29）**: Unit-3 Debate のみ「**論理優位ディベート系（敬語ベース、論理で黙らせるスタイル）**」に再定義。「〜じゃないですか？」「結局〜」「論理的に考えて」「データあるんですか？」型の論理優位トーンを採用。理由: M-1（判断力の弱体化＝論理的自己甘やかし）の効果を最大化。詳細は [Unit-3 business-rules.md DEBATE-05](../../construction/unit-3-debate/functional-design/business-rules.md) 参照。Unit-4/5/6/7/8 のコピートーンは要件書本則（友達系）を維持
- **FR-DEBATE-09** 【M-2: 購買快楽のストレス解消剤化】: 論破プロンプトには **ストレス × ご褒美軸** を必ず併走させる。コンテキスト信号（直近 7 日の会議密度・残業時刻分布・深夜帯利用回数・カレンダー連続予定数）から推定したストレスレベルを `ctx.stress_level = {low/mid/high}` として渡し、mid 以上では「今日のイライラ、このイヤホンで明日リセットしようぜ」型の **ストレス解消としての購買快楽** を明示的に訴求するコピーを生成する。Amazon 遷移完了後は必ず「今日もいい選択だったね。明日の自分、ちょっと機嫌いいはず」型の **肯定フィードバック** をトーストまたはプッシュで返し、ストレス発生 → YUDANE 起動 → 購買 → 快楽のループを強化する。NG-6（脅迫・罪悪感強要）に踏み込む表現（「買わないとストレス悪化するよ」等）は出力モデレーションで除去される
...
- **FR-REEL-03** 【M-2 連動】: 推薦コメントは嗜好 × 時間帯 × **疲労度（ストレスレベル）** × **カレンダー予定** の 4 軸で LLM 動的生成。「確保しておきました」「○○ のために見つけといた」等の所有感訴求ラベルに加えて、ストレスレベルが mid 以上のときは **「今日の会議 6 本、よく戦った。ご褒美は当然じゃね？」型の疲労連動ブーストコピー** を優先表示する。深夜帯（22〜2 時）は高単価帯 (平均の 1.5〜3 倍) を優先投入し、購買 = 即時ストレス解消の脳内回路を強化する（M-2）
...
v0.6 → v0.7 で更新（Inception 仕上げラウンド）:

- **ダメ化効果マトリクス（§2.6）新設**: FR × 5 退化軸の 2 次元表で、各機能要件がどの退化軸を加速するかを可視化。persona-journey.md の Year 1 到達点と 1:1 対応
- **NG シナリオ集（`ng-scenarios.md`）新設**: §9 NG-1〜8 を「発動条件 / YUDANE が採る行動 / 発動しない条件（意図的な逃げ道）/ 倫理的な線引き」の 4 フィールドで実体化
- **市場ポジショニング（`market-positioning.md`）新設**: 「①欲しいかも→②迷う→③買う→④持つ」の 4 フェーズ × 収益構造の自己開示度 × ダメ化到達点と可逆性の 3 軸比較で、YUDANE の独自立ち位置を明らかにする
- **モックアップ検証フェーズ（`mockup-validation/`）正式化**: AI-DLC 公式にない補助ステージを追加し、6 ファイル（mockup-plan / screen-hypothesis-map / dark-copy-inventory / color-rationale / 3-tap-timeline / mockup-to-uc-traceability）でモックアップを「仮説検証の証跡」に昇格
- **逆相性ペルソナ（美咲さん）追加**: `personas.md` に、YUDANE に堕ちない 20% の代表例として悠介の友人美咲（33 歳）を追加。persona-journey.md Year 1 と物語的に接続
- **ローカル環境検証のみに方針確定**: Amazon Approved Mobile Application 申請は予選までに実施しない。予選までの段階はモックデータで要件を満たす。決勝判断は 5/30 予選結果後（§A-10 明確化）

v0.7 → v0.8 で更新（ダメ化メカニズムの 3 段明示化）:

- **ダメ化の 3 段メカニズム（§2.5.1）新設**: 従来の時間軸（Day 1 → Year 1）に加えて、心理メカニズムを M-1（判断力の弱体化・論理的自己甘やかし）/ M-2（購買快楽のストレス解消剤化・ドーパミン依存形成）/ M-3（判断力の完全移譲）の 3 段階で明示。2.5.2 では時間軸と心理メカニズムを 2 次元マトリクスで対応付け
- **FR-DEBATE-09 新設**: ストレス × ご褒美軸を論破プロンプトに併走させる機能要件。`ctx.stress_level` をコンテキスト信号に追加し、mid 以上ではストレス解消としての購買快楽を明示的に訴求。Amazon 遷移後の肯定フィードバック（「今日もいい選択だったね。明日の自分、ちょっと機嫌いいはず」）もここで定義
- **FR-DEBATE-02 拡張**: M-1 の担当機能として明記、心理軸を FR-DEBATE-09 と併走させる前提を追記
- **FR-REEL-03 拡張**: 「所有感訴求」から「疲労連動ブースト（M-2 連動）」に再定義。ストレスレベル mid 以上での疲労連動コピー優先と、深夜帯の高単価優先投入（購買 = 即時ストレス解消の脳内回路強化）を明文化
- **§2.6 ダメ化効果マトリクス拡張**: 5 軸 → 6 軸化（⑥ストレスを購買以外で処理する力 = M-2 軸を追加）。FR-DEBATE-09 / FR-REEL-03 / FR-GAME-01 が M-2 コア 3 機能として整理
- **§0 / §2.1 / §2.2 書き換え**: エグゼクティブサマリーに「ダメ化の 3 段メカニズム」を明記、エレベーターピッチと世界観トーンに「ストレス解消としての購買快楽」「肯定フィードバックによる条件反射形成」を設計意図として追記
...
1. **Workflow Planning**: どの後続ステージを EXECUTE / SKIP するか決定（Units Generation は Unit 分解の適切さの観点から原則 EXECUTE 推奨）

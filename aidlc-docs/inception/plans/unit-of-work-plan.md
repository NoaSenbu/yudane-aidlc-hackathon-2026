...
合計 **9 Units**。各 Unit が **デモで見せられる UC 単位の価値** を持つ。縦割り Unit 構成 + Unit 間依存の明示ができる。
...
合計 **8 Units**。Platform を最初に作って他 Unit を並行させる戦略。ハッカソン向きで、縦割り + Unit 間依存明示ができる。
